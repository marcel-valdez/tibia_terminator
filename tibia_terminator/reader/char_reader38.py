...
# common value
MAGIC_SHIELD_TO_SPEED_OFFSET = 204

parser = argparse.ArgumentParser(
    description=(
        "Fetch the character stats in BASH evaluatable format.\n"
        "The result will be in the format: \n"
        "HP=###;MANA=###;SPEED=###;SOUL_POINTS=###"
    )
)
parser.add_argument(
    "--mana_address", help="Memory address for the mana value.", type=str, default=None
)
parser.add_argument(
    "--hp_address",
    help=(
        "Memory address for the HP value. This value "
...
parser.add_argument(
    "--app_config_path", help="Path to memory addresses", type=str, required=True
)
parser.add_argument(
    "--verbose", help="Show verbose output.", action="store_true", default=False
)
...
class CharReader38:
    ...
    def __fetch_stats(self):
        ...
        try:
            if self.mana_address is not None:
                stats["mana"] = self.memory_reader.read_address_ctype(
                    self.mana_address, c_int()
                )
            if self.hp_address is not None:
                stats["hp"] = self.memory_reader.read_address_ctype(
                    self.hp_address, c_int()
                )
            if self.magic_shield_address is not None:
                stats["magic_shield"] = self.memory_reader.read_address_ctype(
                    self.magic_shield_address, c_int16()
                )
            if self.speed_address is not None:
                stats["speed"] = self.memory_reader.read_address_ctype(
                    self.speed_address, c_int16()
                )
    ...
    def get_max_hp(self):
        ...
        self.memory_reader.open()
        try:
            return self.memory_reader.read_address_ctype(self.max_hp_address, c_int())
        finally:
            self.memory_reader.close()
    ...
    def get_max_mana(self):
        ...
        self.memory_reader.open()
        try:
            return self.memory_reader.read_address_ctype(self.max_mana_address, c_int())
        finally:
            self.memory_reader.close()
    ...
    def init_mana_address(self, override_value=None):
        if override_value is not None:
            ...
            if self.mana_address is None:
                raise Exception("A mana address is required.")

        if self.verbose:
            print(
                "Mana memory address is - {} ({})".format(
    ...
    def init_max_mana_address(self, override_value=None):
        if override_value is not None:
            ...
            if self.mana_address is not None:
                ...
                raise Exception(
                    "Could not find the max mana memory address. "
                    "The hardcoded offset is stale."
                )

        if self.verbose:
            print(
                "Max mana address is - {} ({})".format(
    ...
    def init_max_hp_address(self, override_value=None):
        if override_value is not None:
            ...
            if self.mana_address is not None:
                ...
                raise Exception(
                    "Could not find the max HP memory address. "
                    "The hardcoded offset is stale."
                )

        if self.verbose:
            print(
                "Max HP address is - {} ({})".format(
    ...
    def init_speed_address(self, override_value=None):
        if override_value is not None:
            ...
            # TODO: Implement automated mechanism
            raise Exception(
                "Could not find the speed memory address. The "
                "hardcoded offset is stale."
            )

        if self.verbose:
            print(
                "Speed memory address is - {} ({})".format(
    ...
    def init_magic_shield_address(self, override_value=None):
        ...
        elif self.magic_shield_address is None and self.speed_address is not None:
            self.magic_shield_address = (
                self.speed_address + MAGIC_SHIELD_TO_SPEED_OFFSET
            )
        else:
            # TODO: Implement automated mechanism
        ...
        if self.verbose:
            print(
                "Magic shield memory address is - {} ({})".format(
                    str(self.magic_shield_address), hex(self.magic_shield_address)
                )
            )
    ...
    def init_soul_points_address(self, override_value=None):
        if override_value is not None:
            ...
            # TODO: Implement automated mechanism
            raise Exception(
                "Could not find the soul points memory address. "
                "The hardcoded offset is stale."
            )

        if self.verbose:
            print(
                "Soul points memory address is - {} ({})".format(
...
def main(
    ...
    print(
        f"HP={stats['hp']};MANA={stats['mana']};SPEED={stats['speed']};"
